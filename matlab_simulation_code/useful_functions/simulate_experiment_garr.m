function [trial_table] = simulate_experiment_garr(nTrials,idExperiment)
...
params = struct();
params.ITImean = 4*60; %mean ITI exponential distribution
...
params.states.ISI = (1:floor(ISI_length/dt))';
params.events.ISI = ones(numel(params.states.ISI),1);
max_isi_state = max(params.states.ISI);
params.states.ISI_2 = (max_isi_state+1:max_isi_state+length(params.states.ISI));
...
params.states.ITIwait = (max_isi_state+1:max_isi_state+floor(wait/dt))';
params.events.ITIwait = ones(numel(params.states.ITIwait),1);

%generate trial types
switch idExperiment
    case 'conditioning'
        trialDistribution = {'CSp','CSp2'};
    case 'degradation'
        trialDistribution = {'CSp2degrade','CSdegrade'};
end
...
for i = 1:numTrials
    [s, e] = simulate_trial_garr(trialSequence{i}, params);
    statesCell{i} = s';
    eventsCell{i} = e';
    trialNumbers(i) = i;
