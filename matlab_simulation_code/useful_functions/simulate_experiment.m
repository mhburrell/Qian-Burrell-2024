function [trial_table] = simulate_experiment(nTrials,idExperiment)
...
params.states.ISI = (1:floor(ISI_length/params.dt)+1)';
...
post_reward = 4; %changed here!
pre_reward = reward_delay;
wait = post_reward + pre_reward;
params.states.ITIwait = (max_isi_state+1:max_isi_state+floor(wait/params.dt))';
...
%     temp_table.states = s';
%     temp_table.events = e';
%     temp_table = addTableVariable(temp_table,'trialnumber',i);
%     temp_table = addTableVariable(temp_table,'trialtype',trialSequence{i});
%     trial_table = [trial_table;temp_table];
% end
